...
public class PlotFix {
    ...
    public static void start (Element material, double Ta, double Tm, double r, double lambda, double A, double period, double L) {
        ...
        //plot and show it.
        chart = ChartFactory.createXYLineChart("Temps fix",
                "posició (m)",
                "temperatura (ºC)",
                dataset,
    ...
    public static void save(){
        if(dataset.getSeriesCount()!=0) {
            try {
                ChartUtilities.saveChartAsPNG(new File("Simulacioposicio" + SimBarra.numsaved), chart, SimBarra.FrameWidth, SimBarra.FrameHeight);
                SimBarra.numsaved++;
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    private static void Simul(Element material, double Ta, double Tm, double r, double lambda, double A, double period, double L, XYSeries series){
        ...
        //we load on the data set that we are going to plot.
        for (i = 0; i < N; i++) {
